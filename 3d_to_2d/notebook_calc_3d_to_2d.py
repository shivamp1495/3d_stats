...
xi_all = np.zeros((len(Pk_terms_names), len(znl), len(r_array)))
...
def collect_xi(j_array, return_dict, r_array, k_hres, P_gg_khres_reg):
    for j in j_array:
        return_dict[j] = ptx.Pk2corr_mat(r_array[j], k_hres, P_gg_khres_reg)
...
for j1 in range(len(Pk_terms_names)):
    ...
    if Pk_terms_names[j1] != 'sig4':
        ...
        if do_multiprocess:
            ...
            xi_dict = manager.dict()
            processes = []

            npool = num_pool
            pi_array = np.arange(len(r_array))
            pi_array_split = np.array_split(pi_array, npool)

            for j in range(npool):
                p = multiprocessing.Process(target=collect_xi, args=(pi_array_split[j], xi_dict, r_array, k_hres, P_gg_khres_reg))
                processes.append(p)
                p.start()
            ...
            for pi in range(len(r_array)):
                xi_all[j1, :, pi] =  xi_dict[pi]

        else:
            for k in range(len(r_array)):
                xi_all_f = ptx.Pk2corr_mat(r_array[k], k_hres, P_gg_khres_reg)
                xi_all[j1, :, k] = xi_all_f
