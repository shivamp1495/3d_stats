...
def get_Pktharray(output_nl_grid, klin, knl, Pkzlin, Pnl_kz, usePNL_for_Pk=True, pt_type=None, Pk_terms_names=None,
                  z_array=None, output_xi=False, use_fftlog=False):
    klinlog = np.log(klin)
    ...
    if output_nl_grid:
        ...
        if output_xi:
            ...
            if use_fftlog:
                ...
                r_lin, xitemp = sph(knl, nu=0, q=1.5)(Plin * renorm, extrap=True)
            ...
            for j in range(len(z_array)):
                if use_fftlog:
                    ...
                    _, xitemp = sph(knl, nu=0, q=1.5)(Pnl_kz[j,:] * renorm, extrap=True)
                ...
                if use_fftlog:
                    ...
                    _, xitemp = sph(knl, nu=0, q=1.5)(k2Pnl1[j,:] * renorm, extrap=True)
                    xi_basis = (1 / (2 * np.pi ** 2)) * xitemp
                xi_k2Pk[j, :] = xi_basis
    else:
        Plin = Plin_klin_fpt
        ...
        if output_nl_grid:
            ...
            if output_xi:
                if use_fftlog:
                    ...
                    _, xitemp = sph(knl, nu=0, q=1.5)(temp(knl_log) * renorm, extrap=True)
            ...
            if output_xi:
                if use_fftlog:
                    ...
                    _, xitemp = sph(knl, nu=0, q=1.5)(np.exp(temp(knl_log)) * renorm, extrap=True)
            ...
            if output_xi:
                if use_fftlog:
                    ...
                    rval, xitemp = sph(knl, nu=0, q=1.5)(temp(knl_log) * renorm, extrap=False)
...
def setup(options):
    ...
    Pkth_array, karray = get_Pktharray(output_nl_grid, klin, knl, Pkzlin, Pnl_kz, pt_type=pt_type,
                                       Pk_terms_names=Pk_terms_names, z_array=znl)
