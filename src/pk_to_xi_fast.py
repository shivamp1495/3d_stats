...
def get_PXm_terms(bias_param, Pk_th_array, pt_type=None):
    if pt_type in ['oneloop_eul_bk','lin_oneloop_eul_bk','nohf_oneloop_eul_bk']:
        Pklin, Pk_halofit, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig3nl, k2Pk,Pmm_linp1loop, sig4 = Pk_th_array[0], Pk_th_array[1], \
                                                                                   Pk_th_array[2], \
                                                                                   Pk_th_array[3], Pk_th_array[4], \
                                                                                   Pk_th_array[5], \
                                                                                   Pk_th_array[6], Pk_th_array[7], \
                                                                                   Pk_th_array[8], Pk_th_array[9], Pk_th_array[10]

        b1E, b2E, bsE, b3nl, bk = bias_param[0], bias_param[1], bias_param[2], bias_param[3], bias_param[4]
        if pt_type == 'oneloop_eul_bk':
            P1_mm = Pk_halofit
        elif pt_type == 'lin_oneloop_eul_bk':
            P1_mm = Pklin
        else:
            P1_mm = Pmm_linp1loop
        PXmNL = b1E * P1_mm + (b2E / 2.) * Pd1d2 + (bsE / 2.) * Pd1s2 + (b3nl / 2.) * sig3nl + bk * k2Pk
        PXmNL_terms = [b1E * P1_mm, (b2E / 2.) * Pd1d2, (bsE / 2.) * Pd1s2, (b3nl / 2.) * sig3nl, bk * k2Pk]
...
def get_PXX_terms_bins(bias_param_bin1, bias_param_bin2, Pk_th_array, pt_type=None):
    if pt_type in ['oneloop_eul_bk','lin_oneloop_eul_bk','nohf_oneloop_eul_bk']:
        ...
        PXXNL = (b1E1 * b1E2) * P1_mm + (1. / 2.) * (b1E1 * b2E2 + b1E2 * b2E1) * Pd1d2 + (1. / 4.) * (
        ...
        PXXNL_terms = [(b1E1 * b1E2) * P1_mm, (1. / 2.) * (b1E1 * b2E2 + b1E2 * b2E1) * Pd1d2,
...
def get_xiXX_terms_bins(bias_param_bin1, bias_param_bin2, xi_all_array, pt_type=None):
    if pt_type in ['oneloop_eul_bk','lin_oneloop_eul_bk','nohf_oneloop_eul_bk']:
        xi_lin, xi_halofit, xi_d1d2, xi_d2d2, xi_d1s2, xi_d2s2, xi_s2s2, sig3nl, k2Pk_xi, xi_linp1loop = xi_all_array[0], \
                                                                                           xi_all_array[1], \
                                                                                           xi_all_array[2], \
                                                                                           xi_all_array[3], \
                                                                                           xi_all_array[4], \
        ...
        xi_XXNL = (b1E1 * b1E2) * xi1_mm + (1. / 2.) * (b1E1 * b2E2 + b1E2 * b2E1) * xi_d1d2 + (1. / 4.) * (
                b2E1 * b2E2) * xi_d2d2 + (
        ...
        xi_XXNL_terms = [(b1E1 * b1E2) * xi1_mm, (1. / 2.) * (b1E1 * b2E2 + b1E2 * b2E1) * xi_d1d2,
...
def get_xiXm_terms(bias_param, xi_all_array, pt_type=None):
    if pt_type in ['oneloop_eul_bk','lin_oneloop_eul_bk','nohf_oneloop_eul_bk']:
        ...
        xi_XmNL = b1E * xi1_mm + (b2E / 2.) * xi_d1d2 + (bsE / 2.) * xi_d1s2 + (b3nl / 2.) * sig3nl + bk * k2Pk_xi
        xi_XmNL_terms = [b1E * xi1_mm, (b2E / 2.) * xi_d1d2, (bsE / 2.) * xi_d1s2, (b3nl / 2.) * sig3nl,
                         bk * k2Pk_xi]
